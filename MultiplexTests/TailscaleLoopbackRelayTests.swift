...
/// In-process only: a socketpair-backed `TailscaleRelayRemote` stands in for
/// the tailnet handle, a plain TCP client for Citadel. Reads are poll-bounded
/// so a broken relay fails fast instead of hanging the suite.
final class TailscaleLoopbackRelayTests: XCTestCase {
    /// Wraps one end of a socketpair as the relay's remote — the same seam a
    /// real `ts_tcp_stream` uses, without a node.
    private struct SocketPairRemote: TailscaleRelayRemote {
        let fd: CInt
        func recv(into buffer: UnsafeMutableRawBufferPointer) -> Int {
            var r: Int
            repeat { r = Darwin.read(fd, buffer.baseAddress, buffer.count) } while r < 0 && errno == EINTR
            return r
        }
        func send(_ buffer: UnsafeRawBufferPointer) -> Int {
            var r: Int
            repeat { r = Darwin.write(fd, buffer.baseAddress, buffer.count) } while r < 0 && errno == EINTR
            return r
        }
    ...
    func testRoundTripBothDirectionsThenEOFPropagates() throws {
        ...
        let port = try relay.start(spliceTo: SocketPairRemote(fd: relayEnd))
        ...
        close(client)
        XCTAssertTrue(reachesEOF(testEnd), "client close should reach the spliced remote as EOF")
    }
    ...
    func testCloseBeforeAcceptReleasesRemote() throws {
        let (relayEnd, testEnd) = try makeSocketPair()
        defer { close(testEnd) }
        let relay = TailscaleLoopbackRelay()
        _ = try relay.start(spliceTo: SocketPairRemote(fd: relayEnd))

        relay.close()
        XCTAssertTrue(reachesEOF(testEnd), "close() before any accept must close the remote")
    }

    func testStartAfterCloseThrowsAndClosesRemote() throws {
        let (relayEnd, testEnd) = try makeSocketPair()
        defer { close(testEnd) }
        let relay = TailscaleLoopbackRelay()
        relay.close()
        XCTAssertThrowsError(try relay.start(spliceTo: SocketPairRemote(fd: relayEnd)))
