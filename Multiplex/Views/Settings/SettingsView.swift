...
struct SettingsView: View {
    ...
    private var tailscaleSection: some View {
        TallyFormSection(
            "Tailscale",
            detail: "Use a reusable auth key: every iPhone or iPad becomes its own tailnet node. The key syncs through iCloud Keychain. The optional Headscale control URL stays on this device. Changes apply the next time the embedded node starts."
        ) {
            ...
            TallyFormField("Control URL") {
                TextField(
                    "Optional · Headscale URL",
                    text: $tailscaleControlURL
                )
                .keyboardType(.URL)
            ...
            TallyFormRow {
                HStack(spacing: 12) {
                    TallyLamp(
                        caption: tailscaleStatusCaption,
        ...
        .disabled(
            savedTailscaleConfiguration == nil
                || savingTailscaleConfiguration
        )
    }
