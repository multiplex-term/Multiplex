...
struct ReleaseNoteEntry: Equatable {
    ...
    /// Short noun phrase for the card's "also in 1.3" line. Entries without
    /// one are still counted there, just not named.
    let mention: String?
}
...
/// What 1.3 changed, said once. The launch card renders `highlights` and the
/// full record renders `banks`; both filter by platform, so GLASS is never
...
enum ReleaseNotes {
    ...
    /// itself as a release with its own notes.
    static let version = "1.3"

    /// The one sentence the release notes already lead with.
    static let promise = "Run herdr instead of tmux, select text in any pane, "
        + "and hear from your agents after you’ve walked away."
    ...
    static let allEntries: [ReleaseNoteEntry] = [
        // MARK: Backends
        ReleaseNoteEntry(
            id: "herdr",
            bank: .backends,
    ...
    /// Order matters: the card takes the first `highlightCount` that this
    /// platform gets, so each device's fourth row is the one that is actually
    /// about it (GLASS on Vision Pro, the title bar on iPad).
    static let allHighlights: [ReleaseNoteHighlight] = [
    ...
    static func alsoLine(for platform: ReleaseNotePlatform) -> String? {
        ...
        var sentence = "Also in \(version): " + listed(Array(named), closing: unnamed == 0)
        if unnamed > 0 {
...
enum ReleaseNotesGate {
    ...
    /// Major and minor only: 1.3.0 → 1.3.1 must not re-open the card, and
    /// 1.3 → 1.4 must.
    private static func release(_ version: String) -> (major: Int, minor: Int)? {
        let parts = version.split(separator: ".").prefix(2).map { Int($0) }
        guard let first = parts.first, let major = first else { return nil }
        guard parts.count > 1 else { return (major, 0) }
        guard let minor = parts[1] else { return nil }
        return (major, minor)
    }
}
