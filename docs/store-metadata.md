...
| Current version | 1.4.1 in `project.yml` (1.0 was the first submitted version; 1.1.0 remained unpublished during App Review, so its release-note content was carried forward into 1.2.0) |
...
The same-day triage facts (2026-08-03 readbacks — TestFlight repro, submission
history, and ASC business/tax verification) are release-ops state, not product
documentation — they live in `local-plan/asc-account-state.md` (untracked);
keep them there until the IAP first reaches `APPROVED`.
...
of the public App Store gallery screenshots, so both assets live outside the
public tree in `local-plan/` (untracked).

| Asset | Purpose |
| --- | --- |
| `local-plan/iap-review-screenshot.jpg` | Canonical 2064×2752 JPEG ready for App Store Connect (714,227 bytes) |
