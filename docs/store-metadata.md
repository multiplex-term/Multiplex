...
| Attach/create/close | A tile attaches the full herdr client. Attach creates missing sessions and restarts stopped ones. The deck's New Session can type the selected setup script and agent launch into the fresh pane before attach; close stops then deletes (herdr keeps its protected default session on disk, stopped) |
| New tab in a terminal | On a herdr tab the terminal's `+ TAB` session entry reads **New Tab in Workspace**: it adds a tab to the session's focused workspace, in the focused pane's directory, and herdr focuses it — the attached client is the surface, so no second Multiplex tab opens. Agent entries launch into that new tab. tmux tabs keep New Session and their own Multiplex tab |
